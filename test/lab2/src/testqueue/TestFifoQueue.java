...
public class TestFifoQueue {
	...
	public final void testemptyIterator(){
		...
		assertFalse("Wrong return Value", itr.hasNext());
	}
	...
	public final void oneElement(){
		myIntQueue.offer(1);
		Iterator<Integer> itr = myIntQueue.iterator();
		assertTrue("Wrong Return Value", itr.hasNext());
		assertTrue("Wrong Input",itr.next() == 1);
		assertFalse("Wrong Return Value", itr.hasNext());
		
	}
	@Test
	public final void severalElements() {
		for(int i = 0; i < 10; i++){
			myIntQueue.offer(i);
		}
		Iterator<Integer> itr = myIntQueue.iterator();
		int i = 0;
		while(itr.hasNext()){
			assertTrue("Wrong Number",itr.next() == i );
			i++;
		}
